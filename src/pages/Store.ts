    ...
    switch(action.type){
        ...
        case "set_sto": {
            return state.map(function(curr){
                return {name: curr.name, type: curr.type, label: curr.label, value: curr.value, checked: action.data}
            })
        }
        case "upd_sto": {
            return state.map(function(curr){
                if(curr.value ===  action.value)
                    return {name: curr.name, type: curr.type, label: curr.label, value: curr.value, checked: action.checked}
                else
                   return {name: curr.name, type: curr.type, label: curr.label, value: curr.value, checked: curr.checked}
            })
        }
        case "del_sto": return []
        default: return state;
...
function    p1Reducer(state = i_state.param1, action){
    switch(action.type){
        case "p1": {
            return {
                Номенклатура: action.Номенклатура === undefined ? state.Номенклатура : action.Номенклатура,
        ...
        case "upd_basket":
            return state.map(todo => {
              if (todo.Артикул === action.basket.Артикул) {
                return { ...todo, Количество: action.basket.Количество, Сумма: action.basket.Сумма}
              }
              return todo
            })  
    ...
    switch(action.type){
        case "search": {
            return {
                ...
                Номенклатура: action.Номенклатура === undefined ? state.Номенклатура : action.Номенклатура,
                Пользователь: action.Пользователь === undefined ? state.Пользователь : action.Пользователь            
            }
        }
